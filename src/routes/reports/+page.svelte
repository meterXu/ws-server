<script>
  ...
  function connectWS() {
    ...
    wsRef.onerror = () => {
      // 不主动 close，让浏览器自然关闭后触发 onclose 重连
      wsConnected = false;
    };
  }
...
<svelte:head><title>🌾☠️信使 — 回复</title></svelte:head>

<div class="mb-6">
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    <StatCard label="累计收到" value={totalCount} color="green" />
  ...
  <div class="p-5 flex gap-3 items-start">
    ...
    <div class="flex gap-2.5 items-center pt-0">
      <button onclick={sendTest} disabled={testSending}
        class="px-5 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:bg-purple-300 transition-colors whitespace-nowrap"
      >发送测试</button>
      {#if testResult}
        <span class="text-xs whitespace-nowrap {testResult === '已发送' ? 'text-green-500' : testResult === '发送中...' ? 'text-gray-400' : 'text-red-500'}">{testResult}</span>
      {/if}
    </div>
  </div>
</Panel>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="sticky top-0 bg-gray-50/80 text-left">
            <th class="px-5 py-2.5 text-xs font-medium text-gray-400 w-[100px]">时间</th>
            <th class="px-5 py-2.5 text-xs font-medium text-gray-400 w-[70px]">来源</th>
            <th class="px-5 py-2.5 text-xs font-medium text-gray-400 w-[130px]">来源 IP</th>
            <th class="px-5 py-2.5 text-xs font-medium text-gray-400">消息内容</th>
          </tr>
        </thead>
        <tbody>
          {#each allMessages as r, i}
            ...
            <tr class="border-b border-gray-50 last:border-b-0">
              <td class="px-5 py-2.5 text-xs text-gray-400 whitespace-nowrap align-top">
                {timePart(r.time)}<br><span class="text-[10px] text-gray-300">{datePart(r.time)}</span>
              </td>
              <td class="px-5 py-2.5 align-top">
                <span class="inline-block text-[10px] px-1.5 py-px rounded font-medium whitespace-nowrap
                  {r.source === 'http' ? 'bg-blue-50 text-blue-600' : ''}
                  {r.source === 'ws' ? 'bg-green-50 text-green-600' : ''}
                  {r.source === 'reply' ? 'bg-amber-50 text-amber-600' : ''}
                ">{r.source === 'http' ? 'HTTP' : r.source === 'reply' ? '自动回复' : 'WS'}</span>
              </td>
              <td class="px-5 py-2.5 text-xs font-mono text-purple-600 whitespace-nowrap align-top">{escHtml(r.ip)}</td>
              <td class="px-5 py-2.5 align-top">
                {#if truncated && !expandedRows[i]}
                  <div class="text-xs font-mono text-gray-600 max-h-[60px] overflow-hidden whitespace-pre-wrap">{escHtml(r.body.slice(0, 120))}...</div>
                  <button onclick={() => toggleExpand(i)} class="text-[11px] text-purple-600 hover:underline mt-1 bg-transparent border-none p-0 cursor-pointer">展开</button>
                {:else}
                  <div class="text-xs font-mono text-gray-600 whitespace-pre-wrap">{escHtml(r.body)}</div>
                  {#if truncated}
                    <button onclick={() => toggleExpand(i)} class="text-[11px] text-purple-600 hover:underline mt-1 bg-transparent border-none p-0 cursor-pointer">收起</button>
                  {/if}
                {/if}
                <button onclick={() => copyBody(i)} class="text-[11px] text-gray-400 border border-gray-200 rounded px-1.5 py-px ml-1 hover:text-purple-600 hover:border-purple-300 bg-transparent cursor-pointer">复制</button>
              </td>
            </tr>
          {/each}
        </tbody>
...
<Panel title="自动回复规则">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-3 p-5 border-b border-gray-50 items-start">
    <div class="flex flex-col gap-1 min-w-0">
      <span class="text-xs text-gray-400 font-medium">规则名称</span>
      <JsonEditor bind:this={ruleNameEditor} value={ruleNameValue} onChange={v => ruleNameValue = v} height="84px" />
    </div>
    <div class="flex flex-col gap-1 min-w-0">
      <span class="text-xs text-gray-400 font-medium">正则匹配</span>
      <JsonEditor bind:this={rulePatternEditor} value={rulePatternValue} onChange={v => rulePatternValue = v} height="84px" />
    </div>
    <div class="flex flex-col gap-1 min-w-0">
      <span class="text-xs text-gray-400 font-medium">自动回复内容</span>
    ...
    <div class="flex items-center gap-2 pt-4 md:col-span-3">
      <label class="flex items-center gap-1.5 text-xs text-gray-500 cursor-pointer">
        <input type="checkbox" bind:checked={ruleEnabled} /> 启用
      </label>
      <button onclick={saveRule} class="px-4 py-1.5 bg-purple-600 text-white rounded-md text-xs font-medium hover:bg-purple-700 transition-colors">添加规则</button>
      {#if editingRuleId}
        <button onclick={clearForm} class="px-4 py-1.5 bg-gray-200 text-gray-600 rounded-md text-xs hover:bg-gray-300 transition-colors">取消</button>
      {/if}
    </div>
  </div>
  ...
  {:else}
    <div>
      {#each rules as r}
        ...
        <div class="px-5 py-3 border-b border-gray-50 last:border-b-0 {editing ? 'bg-purple-50/20' : ''}">
          <div class="grid grid-cols-1 md:grid-cols-[1fr_1.5fr_2fr_auto] gap-3 items-start">
            <div class="min-w-0">
              {#if editing}
                <textarea bind:value={ev.name} class="w-full p-2 border border-gray-200 rounded-md text-xs resize-y focus:outline-none focus:border-purple-400 min-h-[84px]"></textarea>
              {:else}
                <span class="font-medium text-sm">{r.name}</span>
                <div class="text-[11px] text-gray-400 mt-0.5">匹配 <span class="text-green-500 font-medium">{r.matchCount}</span> 次 · {timeStr}</div>
              {/if}
            </div>
            <div class="min-w-0">
              {#if editing}
                <textarea bind:value={ev.pattern} class="w-full p-2 border border-gray-200 rounded-md text-xs resize-y focus:outline-none focus:border-purple-400 min-h-[84px]"></textarea>
              {:else}
                <span class="text-xs font-mono text-purple-600 bg-purple-50 px-2 py-0.5 rounded truncate block" title={r.pattern}>{r.pattern}</span>
              {/if}
            </div>
            <div class="min-w-0">
              {#if editing}
                <textarea bind:value={ev.reply} class="w-full p-2 border border-gray-200 rounded-md text-xs resize-y focus:outline-none focus:border-purple-400 min-h-[84px]"></textarea>
              {:else}
                <span class="text-xs text-gray-500 truncate block max-w-[320px]">{r.reply}</span>
              {/if}
            </div>
            <div class="flex items-center gap-1.5 whitespace-nowrap {editing ? 'pt-5' : ''}">
              <Toggle checked={r.enabled} onChanged={v => toggleRule(r.id, v)} />
              {#if editing}
                <button onclick={() => saveEdit(r.id)} class="px-2.5 py-1 bg-purple-600 text-white rounded text-[11px] font-medium hover:bg-purple-700 transition-colors">更新</button>
                <button onclick={() => cancelEdit(r.id)} class="px-2.5 py-1 bg-white text-gray-400 border border-gray-200 rounded text-[11px] hover:bg-gray-50 transition-colors">取消</button>
              {:else}
                <button onclick={() => startEdit(r.id)} class="px-2.5 py-1 bg-gray-200 text-gray-600 rounded text-[11px] font-medium hover:bg-gray-300 transition-colors">编辑</button>
              {/if}
              <button onclick={() => deleteRule(r.id)} class="px-2.5 py-1 bg-white text-red-400 border border-red-200 rounded text-[11px] hover:bg-red-50 transition-colors">删除</button>
            </div>
          </div>
        </div>
      {/each}
